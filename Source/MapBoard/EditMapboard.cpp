...
HRESULT CEditMapBoard::GenerateNewMapA(int nWidth, int nHeight, LPSTR lpLibFilename, LPSTR lpBGFilename){
...
HRESULT CEditMapBoard::GenerateNewMapW(int nWidth, int nHeight, LPWSTR lpLibFilename, LPWSTR lpBGFilename){
	if(nWidth<1)return E_FAIL;
	if(nHeight<1)return E_FAIL;
	if(lpLibFilename==NULL)return E_FAIL;
	...
	//Copy filenames into memory
	wcscpy(m_lpLibraryFilenameW, lpLibFilename);

	//Only copy background if it exist, a background isn't necessary
	if(lpBGFilename!=NULL)
		wcscpy(m_lpBGFilenameW, lpBGFilename);

	//Set width and height
	m_nMapWidth=nWidth;
	m_nMapHeight=nHeight;

	m_lpMapFilenameW[0]=NULL;
...
HRESULT CEditMapBoard::SaveMapA(LPSTR lpMapFilename){
	HANDLE hFile;
	...
	sMapHeader.lLibraryNameSize=(strlen(m_lpLibraryFilenameA)+1)*sizeof(WCHAR);
	sMapHeader.lBGNameSize=(strlen(m_lpBGFilenameA)+1)*sizeof(WCHAR);
...
HRESULT CEditMapBoard::SaveMapW(LPWSTR lpMapFilename){
	HANDLE hFile;
	hFile=CreateFileW(lpMapFilename, GENERIC_WRITE, FILE_SHARE_WRITE, (LPSECURITY_ATTRIBUTES)NULL,
							CREATE_ALWAYS, FILE_ATTRIBUTE_NORMAL, (HANDLE)NULL);
	...
	sMapHeader.nMapHeight=m_nMapHeight;
	sMapHeader.lNumTiles=m_nMapWidth*m_nMapHeight;
	sMapHeader.lLibraryNameSize=(wcslen(m_lpLibraryFilenameW)+1)*sizeof(WCHAR);
	sMapHeader.lBGNameSize=(wcslen(m_lpBGFilenameW)+1)*sizeof(WCHAR);
	sMapHeader.lTileDataSize=sizeof(*m_pTile)*sMapHeader.lNumTiles;
	sMapHeader.lArchDataSize=sizeof(*m_pArch)*sMapHeader.lNumTiles;
	sMapHeader.lObjectDataSize=sizeof(*m_pObject)*sMapHeader.lNumTiles;
	...
	*/
	...
	//Write library name
	WriteFile(hFile, m_lpLibraryFilenameW, sMapHeader.lLibraryNameSize, &dwBytesWritten, NULL);
	//Write background name
	WriteFile(hFile, m_lpBGFilenameW, sMapHeader.lBGNameSize, &dwBytesWritten, NULL);
...
void CEditMapBoard::ChangeBackgroundA(LPCSTR lpBackgroudFilename){
	strcpy(m_lpBGFilenameA, lpBackgroudFilename);
}

void CEditMapBoard::ChangeBackgroundW(LPCWSTR lpBackgroudFilename){
	wcscpy(m_lpBGFilenameW, lpBackgroudFilename);
}

void CEditMapBoard::ChangeLibraryA(LPCSTR lpLibraryFilename){
	strcpy(m_lpLibraryFilenameA, lpLibraryFilename);
}

void CEditMapBoard::ChangeLibraryW(LPCWSTR lpLibraryFilename){
	wcscpy(m_lpLibraryFilenameW, lpLibraryFilename);
}
