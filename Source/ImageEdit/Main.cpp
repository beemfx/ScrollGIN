...
BOOL CALLBACK GenerateProc(HWND hwnd, UINT uMsg, WPARAM wParam, LPARAM lParam){
	...
	case WM_COMMAND:
	{
		...
		case ID_GENERATE:
		{
			...
			if(SUCCEEDED(ILibrary.AutoGenerateData(nBitmap, nWidth, nHeight, nXSpacing, nYSpacing, bClear))){
				EndDialog(hwnd, 1);
				break;
			}else{
				NotifyUser(hwnd, TEXT("A value was invalid"), P_HIGH, EL_FAIL);
...
BOOL CALLBACK BitmapProc(HWND hwnd, UINT uMsg, WPARAM wParam, LPARAM lParam){
	...
	case WM_COMMAND:
	{
		...
		case ID_ADD:
		{
			...
			if(FAILED(ILibrary.AddBitmap(szNewBitmap))){
...
BOOL CALLBACK EditProc(HWND hwnd, UINT uMsg, WPARAM wParam, LPARAM lParam){
	...
	case WM_COMMAND:
	{
		...
		case ID_FILEOPEN:
			...
			if(SUCCEEDED(ILibrary.LoadData(szLastFile))){
		...
		case ID_FILESAVE:
		{
			if(szLastFile[0]==NULL)
				...
				if(FAILED(ILibrary.SaveData(szLastFile)))
		...
		case ID_LIBRARY_BUILD:
		{
			...
			{
				if(SUCCEEDED(ILibrary.BuildLibrary(szBuildFile))){
		...
		case ID_LIBRARY_IMPORT:
		{
			...
			if(!GetOpenFilename(
				...
				if(SUCCEEDED(ILibrary.ImportLibrary(szImportFile))){
