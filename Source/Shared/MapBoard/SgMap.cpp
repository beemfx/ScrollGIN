...
bool SgMap::LoadMap(sg_cpstr lpMapFilename)
{
	...
	std::ifstream File(lpMapFilename, std::ios::binary);
	if (!File.is_open())
	{
		return false;
	}
	...
	File.read(reinterpret_cast<char*>(&sMapHeader), sizeof(sMapHeader));
	if (!File)
	{
		return false;
	}
	...
	sg_char16 szTempLibName[260];
	sg_char16 szTempBGName[260];
	
	//Read and convert lib filename
	File.read(reinterpret_cast<char*>(szTempLibName), sMapHeader.lLibraryNameSize);
	...
	File.read(reinterpret_cast<char*>(szTempBGName), sMapHeader.lBGNameSize);
	WideCharToMultiByte(CP_ACP, 0, szTempBGName, sMapHeader.lBGNameSize/sizeof(sg_char16), m_lpBGFilenameA, MAX_PATH, NULL, NULL);
	//#endif //unicode

	//Read the tile data
	File.read(reinterpret_cast<char*>(m_pTile), sMapHeader.lTileDataSize);
	//Read the architecture data
	File.read(reinterpret_cast<char*>(m_pArch), sMapHeader.lArchDataSize);
	//Read the object data
	File.read(reinterpret_cast<char*>(m_pObject), sMapHeader.lObjectDataSize);
...
{
	std::ofstream File(lpMapFilename, std::ios::binary);
	if (!File.is_open())
	{
		return false;
	}
	...
	//Write header
	File.write(reinterpret_cast<const char*>(&sMapHeader), sizeof(sMapHeader));
	/*
	...
	*/
	...
	//Write wide lib name
	File.write(reinterpret_cast<const char*>(szTempLibName), sMapHeader.lLibraryNameSize);
	//Write wide bg name
	File.write(reinterpret_cast<const char*>(szTempBGName), sMapHeader.lBGNameSize);
	
	//Write tile data
	File.write(reinterpret_cast<const char*>(m_pTile), sMapHeader.lTileDataSize);
	//Write the archtitecture data
	File.write(reinterpret_cast<const char*>(m_pArch), sMapHeader.lArchDataSize);
	//Write the object data
	File.write(reinterpret_cast<const char*>(m_pObject), sMapHeader.lObjectDataSize);
